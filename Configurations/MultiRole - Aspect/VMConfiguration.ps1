...
Configuration AutoLab {
    ...
    node $AllNodes.Where({$_.Role -eq 'DC'}).NodeName {
        ...
        foreach ($user in $Users) {
            ...
            xADUser $user.samaccountname {
                ...
                UserPrincipalName             = "$($user.samaccountname)@$($node.domainname)"
            }
        } #user
    ...
    node $AllNodes.Where({ $_.Role -eq 'MX' }).NodeName {

        $DomainCredential = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList ("$($node.DomainName)\$($Credential.UserName)", $Credential.Password)

        <# file cmtrace {
            DestinationPath = "C:\windows\System32"
            SourcePath = "C:\Program Files (x86)\ConfigMgr 2012 Toolkit R2\ClientTools\CMTrace.exe"
            Type = "file" 
        } #>
        ...
        xPendingReboot BeforeExchangeInstall {
            ...
            DependsOn = '[xADComputer]CHECKINMAN'
        ...
        ExchInstall InstallExchange {
            Path       = 'D:\Setup.exe'
            #Arguments  = '/IAcceptExchangeServerLicenseTerms_DiagnosticDataOFF /P /ON:FIRSTORG'
            Arguments  = '/IAcceptExchangeServerLicenseTerms_DiagnosticDataOFF /M:install /r:mailbox /ON:FIRSTORG /InstallWindowsComponents'
            Credential = $DomainCredential
            DependsOn  = '[xADComputer]CHECKINMAN'
        }
