...
node $AllNodes.Where({$_.Role -eq 'ConfigMgr'}).NodeName {
    ...
    xSccmPreReqs SCCMPreReqs
    {
        ...
        LocalAdministrators    = @("$($node.DomainNetBIOSNAME)\SCCM-Servers","$($node.DomainNetBIOSNAME)\SCCM-CMInstall","$($node.DomainNetBIOSNAME)\Admin")
    ...
    SQLSetup SCCMSqlInstall
    {
        ...
        SQLSysAdminAccounts = @("$($node.DomainNetBIOSNAME)\SCCM-Servers","$($node.DomainNetBIOSNAME)\Admin","$($node.DomainNetBIOSNAME)\SCCM-CMInstall")
        InstallSQLDataDir   = 'C:'
