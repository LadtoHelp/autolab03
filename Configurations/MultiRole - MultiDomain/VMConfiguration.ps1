...
#region Domain Controller config 02

node $AllNodes.Where({$_.Role -eq 'DC2'}).NodeName {

    xWindowsFeatureSet RSAT-AD-Tools {
        Name = "RSAT-AD-Tools"
    ...
    $DomainCredential2 = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList ("$($node.DomainName2)\$($Credential.UserName)", $Credential.Password)
    ...
    $Groups = (Get-Content $PSScriptRoot\AD-Group.json | ConvertFrom-Json)
        ...
        foreach ($user in $Users) {
            ...
            xADUser $user.samaccountname {
                ...
                Department = $User.department
                Enabled = $true
                Password = $DomainCredential2
                DomainAdministratorCredential = $DomainCredential2
                PasswordNeverExpires = $True
                DependsOn = '[xADDomain]FirstDC'
                PasswordAuthentication = 'Negotiate'
            }
    ...
    #prestage Web Server Computer objects
     ...
     #add Web Servers group with Web Server computer objects as members
            ...
            xADGroup WebServerGroup {
                ...
                DependsOn = '[xADOrganizationalUnit]IT'
                Members = $WebServers
                Credential = $DomainCredential2
                Category = 'Security'
                Path = "OU=IT,$($Node.DomainDN2)"
                Ensure = 'Present'
                }

            xADForestProperties AddUPN {
                ForestName = $node.DomainName2
                UserPrincipalNameSuffixToAdd = "$($node.vanitydomain2)"              
            ...
            xDnsServerConditionalForwarder 'Forwarder' {
                Name             = 'mycompany.local'
                MasterServers    = @('192.168.3.10')
                ReplicationScope = 'Forest'
                Ensure           = 'Present'

            }
            xWaitForADDomain Domain01
        {
            DomainName = $node.domainname
        }
            ...
            xADDomainTrust 'Trust' {
                ...
                TargetDomainName = $node.DomainName2
                TargetDomainAdministratorCredential = $DomainCredential
                TrustType = "External"
