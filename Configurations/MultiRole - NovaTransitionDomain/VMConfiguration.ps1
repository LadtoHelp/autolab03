...
Configuration AutoLab {
    ...
    #Import-DSCResource -modulename "ActiveDirectoryDsc" -ModuleVersion "6.5.0.0"
...
<# 
    ...
    node $AllNodes.Where({ $_.Role -eq 'DomainJoinChild' }).NodeName {

        $DomainCredential = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList ("$($node.DomainNameChild)\$($Credential.UserName)", $Credential.Password)
        ...
        xComputer JoinDC {
            ...
            Credential = $DomainCredential
            DependsOn  = '[xWaitForADDomain]DSCForestWait'
            JoinOU     = "OU=IT," + $($Node.DomainDNChild)
        }
    }
