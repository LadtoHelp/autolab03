...
Configuration AutoLab {
    ...
    node $AllNodes.Where({ $_.Role -eq 'DC' }).NodeName {
        ...
        xADGroup AddLabAdmintoRDSUsers {
    ...
    xvmdvddrive AddISO {
        ...
        xADOrganizationalUnit CreateSCCM_OU {
            Name        = 'SCCM'
            Path        = $Node.DomainDN
            Credential  = $DomainCredential
        ...
        xADGroup CreateSCCMGroup03 {
            ...
            Credential       = $DomainCredential
            Path             = "$SCCMOU"
            Description      = "Objects in this Group will be assigned Full Administrator rights within SCCM" 
        }
        ...
        #region Extend Schema
            ...
            <# Script ExtendSchema {
                ...
                SetScript  = {
                    ...
                    Write2Log "Running Set Script as $($DomainCredential.UserName)"
                    ...
                    $root = $root.rootDomainNamingContext
                    ...
                    try {
                        ...
                        Write2log "Failed during SET"
                        Write2Log "$($_.Exception.Message)"
                        Write2Log "$($Error[0])"
                    }
                ...
                TestScript = { 
                    ...
                    try {
                        $OUName = 'System Management'; $Filter = "(&(objectCategory=container)(name=$OUName))"; $dn = New-Object System.DirectoryServices.DirectoryEntry ("LDAP://corp.iconwater.com.au") -ErrorAction SilentlyContinue
                        if ($dn) {
                            $Searcher = New-Object System.DirectoryServices.DirectorySearcher($dn)
                            
                            if ($Searcher) {
                                $Searcher.Filter = $Filter; $ADUserPath = $Searcher.FindOne() ; $ou = $ADUserPath.GetDirectoryEntry()
                            }
                        }
                        else {
                            Write2Log "NO DN FOUND"
                        }
                        
                    }
                    Catch {
                        Write-Verbose "Failed during Test to check for the OU"
                        Write2log "Failed during Test to check for the OU"
                        Write2Log "$($_.Exception.Message)"
                        Write2Log "$($Error[0])"

                    }
                    if ($ou) {
                        Write2log "Found OU $($ou)"
                        Return $True
                    } 
                    Else {
                        Return $false
    ...
    xvmdvddrive AddISO {
        ...
        CMEmailNotificationComponent EmailSettings {
            SiteCode             = $SiteCode
            SendFrom             = "emailsender@$($node.vanitydomain)"
            SmtpServerFqdn       = "EmailServer.$($node.DomainName)"
